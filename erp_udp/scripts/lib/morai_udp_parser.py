...
class udp_parser :
    ...
    def data_parsing(self,raw_data) :
        ...
        #################################################################################### vGen
        elif self.data_type == 'gen_status' :

            header=raw_data[0:11].decode()
            data_length=struct.unpack('i',raw_data[11:15])
                                        
            if header == '#MoraiInfo$' and data_length[0] ==32:
                unpacked_mode = struct.unpack('b',raw_data[15:16])
                unpacked_gear = struct.unpack('b',raw_data[16:17])
                unpacked_new_data = struct.unpack('fi',raw_data[17:25])
                unpacked_data=struct.unpack('ffffffff',raw_data[27:59])

                self.parsed_data=list(unpacked_mode+unpacked_gear+unpacked_new_data+unpacked_data) 
            

        elif self.data_type == 'gen_obj' :
            header=raw_data[0:14].decode()            
...
#################################################################################### vGen_ctrl_init
        elif self.data_type=='gen_ctrl_cmd':
            header='#MoraiCtrlCmd$'.encode()
            data_length=struct.pack('i',12)            
            self.upper=header+data_length            
            self.tail='\r\n'.encode()       
#################################################################################### SetTrafficLight_init
...
#################################################################################### vGen_ctrl
            ...
            # data= [accel,brake,steering_angle]            
...
#################################################################################### SetTrafficLight
            ...
            #data=[automode, trafficlightindex, trafficlightstatus]
...
#################################################################################### Kaist Ghost
        elif self.data_type=='ghost':
            packed_data=struct.pack('IIIIifffff',data[0],data[1],0,data[2],0,0,0,0,0,0)
            print(data)
